...
//       const info = PROJECTS[key];
//       if (info) {
//         const pos = colliderObject.translation();
//         onProjectEnter({
//           ...info,
...
//   // each frame, drive & rotate the player
//   useFrame((_, delta) => {
//     if (!body.current) return;
//     const { forward, backward, leftward, rightward, jump, shift } = getKeys();

//     // 1) build local‐space direction vector
//     const dir = new THREE.Vector3(
//       (rightward ? 1 : 0) - (leftward ? 1 : 0),
//       0,
//       (backward ? 1 : 0) - (forward ? 1 : 0)
...
//     // 2) transform into world space via the body’s quaternion
//     const { x, y, z, w } = body.current.rotation();
//     const worldDir = dir.applyQuaternion(new THREE.Quaternion(x, y, z, w));

//     // 3) choose speed
//     const speed = shift ? 20 : 10;

//     // 4) preserve vertical velocity, set new horizontal velocity
//     const vel = body.current.linvel();
//     body.current.setLinvel(
//       { x: worldDir.x * speed, y: vel.y, z: worldDir.z * speed },
//       true
...
//   // when we hit a building, look up its info & fire the callback
//   const handleCollisionEnter = ({ colliderObject }) => {
//     const name = colliderObject.name;
//     if (name?.startsWith("building-")) {
//       const key = name.replace("building-", "");
...
//   // expose the Rapier body to parent refs (for respawns, camera, etc.)
//   useEffect(() => {
//     if (playerRef) playerRef.current = body.current;
//   }, [playerRef]);
